-- =====================================================
-- KỊCH BẢN SEED DỮ LIỆU SẢN PHẨM & DANH MỤC CHO TECHSHOP
-- CƠ SỞ DỮ LIỆU: techshop_productdb & techshop_inventorydb
-- =====================================================

CREATE DATABASE IF NOT EXISTS techshop_productdb;
...
-- Lưu ý: Định dạng specifications lưu chuỗi JSON đặc tả thông số kỹ thuật.
...
-- Cột 1: Laptop Văn Phòng (category_id = 1)
...
-- Cột 2: Laptop Gaming (category_id = 2)
...
-- Cột 3: PC Đồng Bộ TechShop (category_id = 3)
...
-- Cột 5: Bàn Phím Cơ (category_id = 5)
...
-- Cột 8: Ghế Gaming (category_id = 8)
...
-- Cột 9: Máy Chơi Game (category_id = 9)
...
-- Cột 10: Phụ Kiện (category_id = 10)
...
-- Cột 11: Laptop Gaming bổ sung (category_id = 2)
(20, 'Laptop Gaming Acer Aspire 7 A715-57G-57YJ', '<h3>Tối ưu hóa đa nhiệm với 16GB RAM</h3><p>Không cần phải lo lắng về việc nâng cấp sau khi mua máy, Acer Aspire 7 A715-57G được trang bị sẵn 16GB RAM chạy Dual Channel. Mức dung lượng này giúp bạn thoải mái mở hàng chục tab Chrome tìm tư liệu, vừa làm Photoshop vừa mở game để xả stress mà máy không bao giờ bị đứng hình hay văng ứng dụng do thiếu RAM.</p><h3>Cổng kết nối đa dạng</h3><p>Đáp ứng tốt nhu cầu học tập và thuyết trình, máy cung cấp đầy đủ các cổng kết nối vật lý bao gồm 3 cổng USB Type-A, 1 cổng USB Type-C tích hợp công nghệ Thunderbolt 4, cổng xuất hình HDMI và đặc biệt là cổng mạng LAN RJ45, đảm bảo kết nối internet cáp quang luôn ổn định khi tải game nặng.</p>', 19990000, 17990000, '/product_image/acer_aspire7.png', 'Acer', 'LGM003', 'acer-aspire-7-a715-57g', 2, 1, '{"specs":{"brand":"Acer","screen":"15.6 inch FHD IPS 144Hz","ram":"16GB DDR4 3200MHz","cpu":"Intel Core i5-12450H","storage":"512GB SSD PCIe NVMe","gpu":"NVIDIA GeForce RTX 3050 4GB"},"cardSpecs":[{"value":"16GB RAM / 512GB SSD","label":"RAM / ROM"},{"value":"GeForce RTX 3050 4GB","label":"VGA"},{"label":"Màn hình","value":"15.6 inch FHD 144Hz"}],"detailSpecs":[{"value":"Intel Core i5-12450H (8 Nhân / 12 Luồng, Up to 4.4GHz)","label":"CPU"},{"value":"16GB DDR4 3200MHz (Đã lắp sẵn đủ cho đa nhiệm)","label":"RAM"},{"value":"512GB PCIe NVMe SSD","label":"Ổ cứng"},{"label":"Màn hình","value":"15.6 inch FHD (1920x1080), IPS, 144Hz"},{"value":"NVIDIA GeForce RTX 3050 4GB GDDR6","label":"Card màn hình"},{"value":"Thunderbolt 4, USB 3.2, HDMI, RJ45, Audio Jack","label":"Cổng kết nối"},{"value":"2.1 kg","label":"Trọng lượng"},{"value":"Windows 11 Home","label":"Hệ điều hành"},{"label":"Bảo hành","value":"12 Tháng"}]}', NOW(), NOW());


-- =====================================================
-- SEED DỮ LIỆU TỒN KHO CHO INVENTORY-SERVICE
-- CƠ SỞ DỮ LIỆU: techshop_inventorydb
-- =====================================================

CREATE DATABASE IF NOT EXISTS techshop_inventorydb;
...
-- Tồn kho cho sản phẩm bổ sung
INSERT INTO inventories (product_id, quantity, reserved_quantity, low_stock_threshold, updated_at) VALUES
(20, 30, 0, 3, NOW());

SELECT 'SUCCESSFULLY SEEDED DATA FOR PRODUCT-SERVICE AND INVENTORY-SERVICE' AS RESULT;
